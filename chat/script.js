function showPage(pageId) {
  ...
  if (pageId === "home") {
    ...
    [
      "generatorFileSelected",
      "analyzerFileSelected",
      "estimatorFileSelected",
    ].forEach((id) => {
      const el = document.getElementById(id);
      if (el) {
        el.textContent = "";
...
async function generateStory() {
  ...
  try {
    const res = await fetch(
      "https://chatbot-backend-production-cf70.up.railway.app/api/chatbot-3/requirement-to-story",
      {
        method: "POST",
...
function selectRole(selectedCardElement) {
  ...
  document.getElementById("selectedRole").value =
    selectedCardElement.dataset.role;
}
...
function generateSummary() {
  ...
  fetch(
    "https://chatbot-backend-production-cf70.up.railway.app/api/chatbot-3",
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
async function calculateEstimate() {
  ...
  try {
    const res = await fetch(
      "https://chatbot-backend-production-cf70.up.railway.app/api/chatbot-3/estimate-effort",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  exportButton.addEventListener("click", async () => {
    ...
    try {
      const response = await fetch(
        "https://chatbot-backend-production-cf70.up.railway.app/api/chatbot-3/export-excel",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ story: input }),
